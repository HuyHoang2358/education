...
<body style="background-image: url('{{ asset('/assets/img/home/Decor.png') }}')">
    ...
    <div class="flex justify-center items-center">
        ...
        <div class="bg-white rounded-xl p-5 text-center font-semibold flex flex-col gap-3 w-fit shadow-xl">
            <form id="sign-on-form" action="{{ route('signon') }}" method="POST">
                @csrf
                ...
                <div id="role-select" class="text-blue-secondary flex gap-3">
                    <input value="{{ old('role') }}" class="role" name="role" type="text" hidden>
                    <button type="button"
                ...
                <div id="sign-on" class="relative flex flex-col gap-5 text-start w-full hidden">
                    ...
                    <div class="w-full">
                        ...
                        <input name="name" type="text"
                            ...
                            placeholder="Nhập họ và tên" required value="{{ old('name') }}"/>
                    </div>
                    <div class="w-full">
                        <label class="peer-focus:text-[#FF8040] block text-base font-bold text-[#063140]">
                        ...
                        <input id="email" name="email" type="email"
                            class="peer w-full bg-transparent placeholder:text-slate-400 text-slate-700 border-b-2 border-[#124D59] py-2 transition duration-300 ease focus:outline-none focus:border-[#FF8040]"
                            placeholder="Nhập tên tài khoản hoặc email" required value="{{ old('email') }}"/>
                    </div>
                    <div class="w-full">
                        <div class="relative">
                    ...
                    <div>
                        <div class="flex items-center gap-1 font-medium text-light-gray">
                            <input name="terms" type="checkbox" class="hover:cursor-pointer scale-110" required {{ old('terms') ? 'checked' : '' }}>
                            <p>Tôi đồng ý với các điều khoản <a href="" class="text-[#1877F2] underline">tại đây</a>
                            </p>
                        </div>
                    </div>
                    <!-- Submit button -->
            ...
            <div id="social-signon-form" class="flex gap-3 text-light-gray hidden">
            ...
            <p class="text-light-gray text-sm">Đã có tài khoản? <a class="text-[#1877F2] underline" href="{{ route('login') }}">Đăng nhập</a></p>
        </div>
    ...
    <script>
        ...
        @if ($errors->any())
            @foreach ($errors->all() as $error)
                toastr.error("{{ $error }}", "Lỗi đăng ký");
            @endforeach
            toggleSignOnForm("{{ old('role') }}");
        @endif
    </script>
</body>
